...
**SHIPD** wordmark plays, then a multi-select over the registry's twelve
...
| `/s:copilot` | Set up the semantic review gate in a repository end to end: preflight the prerequisites, install the four managed files with `shipd copilot add`, commit and push them, then — in one batched consent round — require the `semantic-review` check, enable auto-merge, and optionally turn strictness on. The reviewer token stays a hand-off, and `shipd doctor` verifies what was set up. |
