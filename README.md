...
| `/s:review` | An AST-aware semantic review of local changes, in the style of popular code-review tools, against a base ref before they are pushed: changed files mapped into cohorts, findings reported by cohort with a high/medium/low rubric and a ship-it/fix-required verdict. Read-only, with a `--json` mode for the PR gate. |
| `/s:drive` | Drive a real browser with Playwright to operate a running app: resolve a target and its cached login, start a one-browser session daemon, probe the live DOM before authoring any selector, drive the requested steps while waiting for named completion signals — never a fixed sleep — and end on a `PASS`/`FAIL` verdict backed by console and network evidence. Optionally records and post-processes a branded, fast-forwarded demo video of the same run. |
...
| Invocation | What it does |
| --- | --- |
| `/s:ask` | Query the oracle before interrupting a human: shape a request into one compact decision, consult the workspace wiki and the repo's spec surfaces, and relay the verdict — a cited recommendation, or a question queued for a person to answer later. |
