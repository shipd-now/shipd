...
on the title row** — no other header control. Activating the
menu control SHALL NOT toggle the group's collapsed state; it SHALL open a
compact **action menu** for that epic — **anchored to the menu control and
not dimming the board**, presenting its actions as a **keyboard-navigable
list with no title bar and no ✕ control** — offering **View epic** always and
...
`Escape` or a **click outside the menu box**. The header's title row SHALL
reserve the menu control's cells **in the title itself, never in the group's
collapsible box**: the menu control SHALL be positioned on the row's own
**menu layer**, docked to the row's trailing edge with a **one-cell inset**
off the lane's scrollbar gutter, so the collapsible box beneath it — which
holds the group's member cards as well as its title — keeps the lane's **full
content width** and **no member card yields a cell to the control**; and the
title (collapse arrow plus title text) SHALL reserve exactly the control's
width plus that inset as its own trailing padding and, when too wide, SHALL
truncate with an **ellipsis inside its own space**, so the title's **content
area ends at or before the control's leading edge**. That positioning SHALL be
**static — no construction-time offset and no resize-time clamp** — so title
text is never laid out or painted beneath the control at any lane width or
collapsed state. The **per-lane card count** SHALL render inside the title as a
...
#### Scenario: The menu control floats at the title row's end
- **GIVEN** the grouping mode is `epic` and any epic's group header
- **WHEN** the header renders
- **THEN** the `≡` menu control sits on the title row inside the group's panel
  band, one row high and three cells wide, docked to the row's trailing edge
  one cell clear of the lane's scrollbar gutter, not overlapping the title's
  content area, with no other header control present
...
- **THEN** the group's collapsible box spans the lane's full scrollable content
  width, so no member card is narrowed by the menu control's cells
...
- **THEN** the title truncates with an ellipsis inside its own space, its
  content area ending at or before the menu control's leading edge, while the
...
- **WHEN** the user presses `Escape` or clicks the board outside the menu box
- **THEN** the menu closes, no run is dispatched, and no modal is pushed

#### Scenario: The action menu opens anchored without dimming the board
- **GIVEN** an epic group header's menu control
- **WHEN** the menu control is activated
- **THEN** the action menu box renders near that control rather than centred,
  and the board behind it is not dimmed (the menu screen has no dimming
  background)
