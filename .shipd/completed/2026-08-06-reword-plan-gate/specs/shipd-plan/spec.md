...
question — "We have enough details — shall I write the plan now?" —
carrying no numbered options and no AskUserQuestion. Because that question
asserts readiness, the skill SHALL present it only when no genuinely open
task-shaping question remains; whenever one does, the skill SHALL take the
`OPEN QUESTIONS` ending instead rather than assert readiness. That question
SHALL drive a loop: an affirmative reply advances to the depth gate; any
other reply (further questions, new information, or scope changes) SHALL be
folded in as continued planning — the skill keeps planning, shows a
one-line delta of what changed, and re-asks "We have enough details —
shall I write the plan now?" once the plan is settled again — looping until
the user affirms. An explicit go-ahead already present in the user's reply
(for example, a bare go-ahead answering an `OPEN QUESTIONS` list) SHALL
count as the affirmative and skip a redundant re-ask. No AskUserQuestion
SHALL be issued in the investigation turn, neither ending SHALL contain any
planning decision, and no depth-gate verdict SHALL appear in the
investigation turn. Internal reasoning SHALL NOT substitute for the digest.
...
#### Scenario: No open questions ends the turn on the readiness go-ahead
- **WHEN** investigation leaves no genuinely open task-shaping question
- **THEN** the digest ends on the single plain-text question "We have enough
  details — shall I write the plan now?" with no numbered options, no
  `OPEN QUESTIONS` header, and no AskUserQuestion

#### Scenario: A non-affirmative reply keeps planning and re-asks
- **WHEN** the user answers "We have enough details — shall I write the plan
  now?" with further questions, new information, or a scope change rather
  than an affirmative
- **THEN** the skill folds the reply in as continued planning, shows a
  one-line delta of what changed, and re-asks "We have enough details —
  shall I write the plan now?" once the plan is settled again, without
  firing the depth gate
...
- **THEN** the skill treats it as the affirmative and advances to the depth
  gate without re-asking "We have enough details — shall I write the plan
  now?"
...
The affirmative to "We have enough details — shall I write the plan now?"
SHALL be the sole approval for a clean gate: when the user affirms and the
depth gate opens no further interactive rounds (the fast path — the gate
needs no more info), the skill SHALL proceed directly to emission without
presenting a shared-understanding summary or an "emit" confirmation. When
instead the depth gate needs more info and the grill loop runs (the depth
path), the skill SHALL, when that loop ends, present a shared-understanding
...
"emit" the recommended option, before proceeding to emission. The fast path
SHALL NOT add this step.

#### Scenario: Clean gate emits on the affirmative alone
- **WHEN** the user affirms "We have enough details — shall I write the plan
  now?" and the depth gate opens no interactive rounds
