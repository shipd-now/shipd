...
`background-tasks` — and `HARNESSES`, fourteen harness entries
(`claude-code`, `cursor`, `github-copilot`, `windsurf`, `aider`, `codex`,
`cline`, `roocode`, `continue`, `antigravity`, `devin`, `oh-my-pi`,
`opencode`, `pi`) each carrying:
...
- **THEN** its `repo_pattern` is `.opencode/commands/shipd-{command}.md`,
  its `user_dir` is `~/.config/opencode/commands/`, its dialect is `yaml`
  with frontmatter `("description",)`, and its `features` are exactly
  `subagents` and `file-references`

#### Scenario: Pi carries its researched paths
- **WHEN** `get("pi")` is read
- **THEN** its `repo_pattern` is `.pi/prompts/shipd-{command}.md`, its
  `user_dir` is `~/.pi/agent/prompts/`, its dialect is `yaml` with
  frontmatter `("description", "argument-hint")`, and its `features` are
  exactly `("file-references",)` — `subagents`, `question-dialogs`, and
  `background-tasks` are undeclared

#### Scenario: Pi and oh-my-pi are separate entries
- **WHEN** `ids()` is read
- **THEN** it contains both `oh-my-pi` and `pi`, and their `repo_pattern`
  values differ (`.omp/commands/…` and `.pi/prompts/…`)
...
- **THEN** stdout contains all fourteen registry ids and the exit code is 0
...
- **THEN** stdout parses as JSON with fourteen entries whose ids match
  `ids()`
