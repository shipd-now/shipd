...
panel; and the **per-lane card count** renders as the row's **final
segment, after the controls**. A header control SHALL never overlap the
title or the count, never be painted over clipped text, and never render
...
- **THEN** the header row carries the count 2 as its final segment, after the
  controls
...
- **THEN** the run control and then the open control sit as flex children on
  ...
  row high and three cells wide, with the count after them, and none of them
...
- **THEN** the title truncates with an ellipsis inside its own space while the
  run/open controls and the count render fully inside the lane, not
  overlapping any text, and remain clickable
...
the group header's run control dispatches and dismiss the modal. The banner's
content SHALL be inset **three cells from the banner's left edge**, and the
banner SHALL be separated from the content below it by **one blank row**. A
...
#### Scenario: The banner is inset and separated from the list below
- **GIVEN** a stalled epic
- **WHEN** its epic-detail modal opens
- **THEN** the banner's text starts three cells from the banner's left edge
  and one blank row separates the banner from the member list below it
