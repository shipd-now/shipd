...
control first (when present), then the open control, packed adjacently inside
the group's panel — never docked at the far edge of the lane row outside the
group's visual box. The header controls and every modal **close (✕) control**
SHALL be **compact — a single row high** — while remaining visually evident as
clickable buttons. The **epic-detail modal** SHALL occupy the
...
initiative when set, a **list of the epic's member specs** — each with its board
state and risk — and the epic's own overview rendered as **Markdown** from its
...
- **THEN** the run control occupies the header's title row immediately after
  the title text with the open control packed immediately after it, inside the
...
- **THEN** that control occupies a single row
