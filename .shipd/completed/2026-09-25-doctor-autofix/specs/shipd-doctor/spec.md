...
`copilot-secret` among the parsed check names. This capability governs the skill's own consent-gated remedies and SHALL
NOT be read as forbidding the CLI an autonomous mode of its own: the
binary's `doctor --fix` (shipd-cli doctor-verb) provisions local tooling
without asking, and the two capabilities divide by what they may touch
rather than by who runs first. Every GitHub-side mutation in the table
above — the protection writes and the `automerge` PATCH — SHALL remain
this skill's alone, consent-gated as specified, and SHALL never be
performed by the CLI's autonomous mode.
...
#### Scenario: The CLI's autonomous mode performs no GitHub mutation
- **WHEN** the binary's `doctor --fix` runs against a repository whose
