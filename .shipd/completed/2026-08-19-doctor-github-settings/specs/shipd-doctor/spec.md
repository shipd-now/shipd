...
declared pipeline — with the pydantic row's remedy standing as the fix when
the pipeline failure's detail names missing pydantic. The skill SHALL
recognize `pipeline`, `difft`, `protection`, `automerge`, and
