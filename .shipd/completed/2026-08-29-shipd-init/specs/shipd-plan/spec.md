...
layout existed. When the user accepts the scaffold, the skill SHALL create
the layout by running the engine's `spec_status.py init` verb — the same
function behind `shipd init` — never by creating the directories by hand.
...
- **THEN** the skill runs `spec_status.py init`, which creates the three
